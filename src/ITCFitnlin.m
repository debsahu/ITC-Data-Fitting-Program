function [outputfinal,p1,p2]=ITCFitnlin(fname,V0,SyC,T,npd,startka,startn,corrected,ptype)
%% ITC Data fitting %%
...
s0=[startn startka ndh(1)-ndh(length(ndh)) mean(ndh)];
[beta,R,J,CovB]=nlinfit( [xmt, mt], ndh, @(par,x)onesitemodelf(par,x(:,1),x(:,2),injv,V0,SyC,I0), s0);
stdcv=sqrt(diag(CovB))';

fitresult.n=beta(1); fitresult.Ka=beta(2); fitresult.dH=beta(3); fitresult.c=beta(4);
std.n=stdcv(1); std.Ka=stdcv(2); std.dH=stdcv(3); std.c=stdcv(3);
...
outputfinal=[Kd erKd fitresult.Ka std.Ka fitresult.n std.n fitresult.dH std.dH fitresult.c];
%% Plot fit with data.
