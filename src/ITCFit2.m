function [outputfinal,p1,p2]=ITCFit2(fname,V0,SyC,T,npd,startka,startn,corrected)
%% ITC Data fitting %%
% ITC fitting of data using one set of binding sites based on Origin fitting equations
...
ft = fittype( ['onesitemodelf2([n,Ka,dH],x , y, [' num2str(injv') '] ,' num2str(V0) ',' num2str(SyC) ', [' num2str(I0) '])'] , 'independent', {'x', 'y'}, 'dependent', 'z' );
opts = fitoptions( ft );
...
opts.MaxIter = 10000;
opts.StartPoint = [startka ndh(1)-ndh(length(ndh)) startn];% Order Ka dH n
...
outputfinal=[Kd erKd fitresult.Ka std(1) fitresult.n std(3) ...
    fitresult.dH std(2)]; % Fitted results sent to output
%% Plot fit with data.
