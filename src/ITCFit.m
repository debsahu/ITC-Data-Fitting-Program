function [outputfinal,p1,p2]=ITCFit(fname,V0,SyC,T,npd,startka,startn,corrected)
%% ITC Data fitting %%
% ITC fitting of data using one set of binding sites with an offset for final titration points
...
ft = fittype( ['onesitemodelf([n,Ka,dH,c],x , y, [' num2str(injv') '] ,' num2str(V0) ',' num2str(SyC) ', [' num2str(I0) '])'] , 'independent', {'x', 'y'}, 'dependent', 'z' );
opts = fitoptions( ft );
...
opts.MaxIter = 10000;
opts.StartPoint = [startka -mean(ndh) abs(max(ndh)-min(ndh)) startn];% Order Ka c dH n
opts.Upper = [Inf Inf Inf Inf];

%% Fit model to data.
[fitresult, gof] = fit( [xmt, mt], ndh, ft, opts ); % Does the Fitting
ci = confint(fitresult,0.68); % Obtains upper/lower bounds for Sigma/STD
std=(abs(ci(1,:)-ci(2,:))/2); % Calculates Sigma/STD

Kd=1/fitresult.Ka;
erKd=std(1)/(fitresult.Ka)^2; % Error Propogation of Ka to get errors of Kd
dS=-(-1.9858775*(273.15+T)*log(fitresult.Ka)-fitresult.dH)/(273.15+T); %dS = -(dG-dH)/T; dG = -RTln(Ka);

%% Uncomment these lines to display results on screen
% fitresult % Displays Fitted Result on Screen
% sprintf('\nRegreesion Coefficient (Adjusted for DOF) : %1.2f \n',gof.adjrsquare)
% ['Kd = (' sprintf('%5.2f',Kd*1e6) ' ± ' sprintf('%5.2f',erKd*1e6) ' ) x 10^{-6} M']
% ['n = ' sprintf('%5.2f',fitresult.n) ' ± ' sprintf('%5.2f',std(4)) ]
% ['dH = ' sprintf('%5.2f',fitresult.dH*1e-3) ' ± ' sprintf('%5.2f',std(3)*1e-3) ' x kcal/mol']

outputfinal=[Kd erKd fitresult.Ka std(1) fitresult.n std(4)  ...
    fitresult.dH std(3) fitresult.c ]; % Fitted results sent to output
%% Plot fit with data.
...
yfitf=onesitemodelf([fitresult.n fitresult.Ka fitresult.dH fitresult.c],...
...
p1=horzcat(xmtf,yfitf);
p2=horzcat(xmt,ndh);

% % Comment/Uncomment to print fitted values on figure
% x=0.95*mean(xmt);
% y1=1.75*mean(ndh);
% text(x,y1,{['K_{d} = (' sprintf('%5.2f',Kd*1e6) ' \pm ' sprintf('%5.2f',erKd*1e6) ' ) x 10^{-6} M'],['n = ' sprintf('%5.2f',fitresult.n) ' \pm ' sprintf('%5.2f',std(4)) ],['\Delta' 'H = ' sprintf('%5.2f',fitresult.dH*1e-3) ' \pm ' sprintf('%5.2f',std(3)*1e-3) ' kcal/mol'],['\Delta' 'S = ' sprintf('%5.2f',dS) ' cal/mol']},'EdgeColor','red');
% hold off;
